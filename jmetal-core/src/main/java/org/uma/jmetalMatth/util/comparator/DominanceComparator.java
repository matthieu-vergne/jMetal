 ...
 * <code>Solution</code> objects) based on a constraint violation, as done in
 * NSGA-II.
 */
public class DominanceComparator<Encoding extends ConstrainedEncoding & ObjectiveBasedEncoding>
		implements Comparator<Solution<?, Encoding>> {
	private OverallConstraintViolationComparator<Encoding> constraintViolationComparator = new OverallConstraintViolationComparator<>();
	 ...
	 */
	@Override
	public int compare(Solution<?, Encoding> solution1,
			Solution<?, Encoding> solution2) {
	...
	private int dominanceTest(Solution<?, Encoding> solution1,
			Solution<?, Encoding> solution2) {
		int result;
		...
		for (int i = 0; i < solution1.getEncoding().getNumberOfObjectives(); i++) {
			value1 = solution1.getEncoding().getObjective(i);
			value2 = solution2.getEncoding().getObjective(i);
			if (value1 < value2) {
				flag = -1;
